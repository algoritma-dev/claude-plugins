...
setup_repo() {
    ...
    mr_fixture opened false
    printf '[]\n' > "$STUB_GLAB_REPLY"
    unset STUB_GLAB_REPLY_REPO CLAUDE_REVIEW_TRUSTED_AUTHORS CLAUDE_REVIEW_REMOTE

    CI_PROJECT_ID=42
    CI_MERGE_REQUEST_IID=7
    CI_MERGE_REQUEST_DIFF_BASE_SHA="$BASE"
    CI_COMMIT_SHA="$HEAD_SHA"
    export CI_PROJECT_ID CI_MERGE_REQUEST_IID CI_MERGE_REQUEST_DIFF_BASE_SHA CI_COMMIT_SHA
    unset CI_MERGE_REQUEST_SOURCE_BRANCH_SHA
}

# Writes the merge request fixture: state, draft flag, and the diff refs the
# API reports, BASE..HEAD_SHA unless given.
mr_fixture() {
    printf '{"project_id":42,"iid":7,"state":"%s","draft":%s,"title":"Add average","description":"Averages numbers.","web_url":"https://git.example.it/team/app/-/merge_requests/7","sha":"%s","diff_refs":{"base_sha":"%s","start_sha":"%s","head_sha":"%s"}}\n' \
        "$1" "$2" "${4:-$HEAD_SHA}" "${3:-$BASE}" "${3:-$BASE}" "${4:-$HEAD_SHA}" > "$STUB_GLAB_REPLY_MR"
}

# Leaves the CI job: none of the pipeline variables are set.
local_machine() {
    unset CI_PROJECT_ID CI_MERGE_REQUEST_IID CI_MERGE_REQUEST_DIFF_BASE_SHA CI_COMMIT_SHA CI_MERGE_REQUEST_SOURCE_BRANCH_SHA
}

# Prints the range the script resolves, or nothing when it resolves none.
range_of() {
    sh "$SCRIPT" "$@" | jq -r '.range'
}
...
set +e
actual=$(sh "$SCRIPT" 7 2>/dev/null | jq -r '.range')
status=$?
set -e
...
# The output carries what the later steps need.
setup_repo
out=$(sh "$SCRIPT" 7)
check "the output names the project ID" "42" "$(printf '%s' "$out" | jq -r '.project_id')"
check "the output carries the head as to" "$HEAD_SHA" "$(printf '%s' "$out" | jq -r '.to')"
check "the output carries the base as from" "$BASE" "$(printf '%s' "$out" | jq -r '.from')"
check "the output carries the title" "Add average" "$(printf '%s' "$out" | jq -r '.title')"
check "the output carries the description" "Averages numbers." "$(printf '%s' "$out" | jq -r '.description')"
check "the output carries the web URL" "https://git.example.it/team/app/-/merge_requests/7" "$(printf '%s' "$out" | jq -r '.web_url')"

# On a developer's machine: no pipeline variables, project given as a path.
setup_repo
local_machine
actual=$(range_of 7 team/app)
check "a local review takes the range from the API" "$BASE..$HEAD_SHA" "$actual"
check_contains "a project path is encoded in the URL" "projects/team%2Fapp/merge_requests/7" "$(cat "$STUB_GLAB_CALLS")"
check_contains "later calls use the numeric project ID" "projects/42/merge_requests/7/notes" "$(cat "$STUB_GLAB_CALLS")"

# Without a project, the clone's own project is used.
setup_repo
local_machine
STUB_GLAB_REPLY_REPO="$WORK/repo.json"
export STUB_GLAB_REPLY_REPO
printf '{"id":42,"path_with_namespace":"team/app"}\n' > "$STUB_GLAB_REPLY_REPO"
actual=$(range_of 7)
check "a local review without a project uses the clone's project" "$BASE..$HEAD_SHA" "$actual"
...
status=$?
set -e
check "no project and no GitLab clone exits 1" "1" "$status"
check_contains "no project and no GitLab clone asks for the project" "pass the project" "$message"

# The CI job of another merge request must not lend its variables.
setup_repo
CI_MERGE_REQUEST_IID=8
CI_COMMIT_SHA="$MID"
export CI_MERGE_REQUEST_IID CI_COMMIT_SHA
actual=$(range_of 7)
check "another merge request's pipeline variables are ignored" "$BASE..$HEAD_SHA" "$actual"

# A clone that lacks the merge request's commits fetches them.
setup_repo
local_machine
REMOTE="$TEST_TMP/remote-$$.git"
git init -q --bare "$REMOTE"
git push -q "$REMOTE" "$HEAD_SHA:refs/merge-requests/7/head"
git push -q "$REMOTE" "$BASE:refs/heads/main"
git --git-dir="$REMOTE" symbolic-ref HEAD refs/heads/main
CLONE=$(mktemp -d "$TEST_TMP/XXXXXX")
# file:// forces the real transport; a plain path would copy every object.
git clone -q "file://$REMOTE" "$CLONE"
cd "$CLONE"
check "the fixture clone lacks the head" "1" "$(if git cat-file -e "$HEAD_SHA^{commit}" 2>/dev/null; then echo 0; else echo 1; fi)"
actual=$(sh "$SCRIPT" 7 42 2>/dev/null | jq -r '.range')
check "a clone without the commits fetches the merge request" "$BASE..$HEAD_SHA" "$actual"
check "the fetched head is in the clone" "0" "$(if git cat-file -e "$HEAD_SHA^{commit}" 2>/dev/null; then echo 0; else echo 1; fi)"

# A clone of some other project cannot fetch the commits and must say so.
setup_repo
local_machine
mr_fixture opened false "$BASE" 0123456789abcdef0123456789abcdef01234567
git remote add origin "$TEST_TMP/nowhere.git"
set +e
message=$(sh "$SCRIPT" 7 42 2>&1 >/dev/null)
status=$?
set -e
check "unreachable commits exit 1" "1" "$status"
check_contains "unreachable commits ask for a clone of the project" "clone of project 42" "$message"

# Outside any git repository there is nothing to diff.
setup_repo
local_machine
NOT_A_REPO=$(mktemp -d "$TEST_TMP/XXXXXX")
cd "$NOT_A_REPO"
set +e
message=$(GIT_CEILING_DIRECTORIES="$TEST_TMP" sh "$SCRIPT" 7 42 2>&1 >/dev/null)
status=$?
set -e
check "outside a git repository exits 1" "1" "$status"
check_contains "outside a git repository says so" "not inside a git repository" "$message"
cd "$WORK"

# A review posted from another account counts once that account is trusted,
# so a local run picks up where the CI bot left off.
setup_repo
notes_fixture "ci-bot:$MID"
actual=$(range_of 7)
check "an untrusted account's marker is ignored" "$BASE..$HEAD_SHA" "$actual"
CLAUDE_REVIEW_TRUSTED_AUTHORS="someone,ci-bot"
export CLAUDE_REVIEW_TRUSTED_AUTHORS
actual=$(range_of 7)
check "a trusted account's marker is honoured" "$MID..$HEAD_SHA" "$actual"
...
setup_repo
printf '{"project_id":42,"state":"opened","work_in_progress":true}\n' > "$STUB_GLAB_REPLY_MR"
set +e
